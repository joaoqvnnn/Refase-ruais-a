...
async def create_pix_payment(valor: float, user_id: int, description: str = "Recarga Larizinha Store") -> dict:
    ...
    import uuid
    return {
        "codigo_pix": "00020101021226830014BR.GOV.BCB.PIX2561qrcodespix.sejaefi.com.br/v2/"
                      + uuid.uuid4().hex,
        "qr_code_base64": None,
        "txid": None,
        "data_expiracao": expiration,
...
async def check_payment_status(payment_id: str) -> str:
    ...
    # Se não houver gateway real, retornamos pendente por padrão
    return "pendente"
...
async def verify_pending_payments():
    ...
    async with async_session() as session:
        ...
        for pagamento in pagamentos:
            ...
            # Se houver txid, consulta status real (quando gateway real)
            if pagamento.txid:
                status = await check_payment_status(pagamento.txid)
            else:
                # Sem txid, não podemos verificar; mantém pendente
                continue
